...
function init_bloque(id_user) {
	$.post('reload/profil.php', {id:id_user}, function(data) {
		document.getElementById(id_user+"_map").innerHTML = data;
	});
}


function get_pro(e) {
	let es = document.querySelector('.leaflet-popup-content');
	if (es) {
		es.style.width = 'fit-content';
		es.style.textAlign = 'center';
	}
	...
	if (e.target.id != "mapid") {
		...
		if (parseInt(id) > 0)
			init_bloque(id);
		if (id == "signal" || id == "bloq" || e.target.classList[0] == "matchs" || e.target.classList[0] == "msg") {
			set_signal(id);
			set_bloque(id);
			set_match(e.target.classList[0]);
			set_msg(e.target.classList[0]);
		}
	}
}

function set_signal(ids) {
...
function msg_show_time(list_id, id, id_go) {
	...
	$.post("reload/msg.php", {id:id}, function(data) {
		...
		if (document.getElementById(id)) {
			...
			if (an_nb !== nb) {
				if (document.getElementById(id+"_txt_msg")) {
